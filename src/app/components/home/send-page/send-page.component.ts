...
export class SendPageComponent implements OnInit {
  ...
  createForm() {
    ...
    this.linkFormGroup = this.formBuilder.group({
      ...
      isPasswordProtected: [true],
      prefix:[this.IL_PREFIX, [Validators.required]],
  ...
  transfer() {
    ...
    this.apiService.emailExist(contacts).subscribe(
      (res: SailsResponse) => {
        ...
        let selectedNonExists = [];
        // If non exists
        if (body.nonExistEmails.length > 0) {
          const settingsConfig = new MatDialogConfig();
          settingsConfig.autoFocus = false;
          if (!this.mobile) {
            settingsConfig.height = '700px';
            settingsConfig.width = '900px';
          } else {
            settingsConfig.width = '90vw';
            settingsConfig.height = '70vh';
          }
          settingsConfig.data = body.nonExistEmails;
          settingsConfig.height = '50vh';
          ...
          this.dialog.open(NonCopaUsersDialogComponent, settingsConfig).afterClosed().subscribe(res => {
            if (res) {
              this.dialog.open(MultiFactorAuthenticationDialogComponent, settingsConfig).afterClosed().subscribe(res => {
                selectedNonExists = res;
              })
            } else {
              // Open fail modal
              this.openFailedModal(settingsConfig).subscribe( (res: {result: boolean, data: any}) => {
                  if(res.result){
                    // Try send again.
                    return res.result
                  } else {
                    // Remove failed contacts from request.
                    this.selectedContacts = this.selectedContacts.filter( contact => contact.id !== res.data.id)
        ...
        // If all are contacts (All Exists)
        else {
          ...
          if (this.user.offlineMode === 'ask') {
            ...
            this.dialog.open(TransferMethodDialogComponent, settingsConfig).afterClosed().subscribe( res => {
              if(res) {
                ...
                // Cancel on transfer method
                settingsConfig.data = _.values(existEmails);
                this.openFailedModal(settingsConfig).subscribe( (res: {result: boolean, data: any}) => {
                  if(res.result){
                    // Try send again.
                    return res.result
  ...
  openFailedModal(config): Observable<any> {
    config.height = '50vh';
    config.width = '50vw';
    return this.dialog.open(FailedDialogComponent, config).afterClosed();
    // this.dialog.open(FailedDialogComponent, config).afterClosed().subscribe( (res: {result: boolean, data: any}) => {
    //   console.log(res);
    //   if(res.result){
    //     // Try send again.
    //     return res.result
